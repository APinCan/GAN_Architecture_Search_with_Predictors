...
def get_activations(images, sess, batch_size=50, verbose=False):
    ...
    d0 = images.shape[0] # 5000

    if batch_size > d0:
        print("warning: batch size is bigger than the data size. setting batch size to data size")
        batch_size = d0
    n_batches = d0 // batch_size # 5000 // 50
...
def calculate_frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-6):
    ...
    cal_diff = diff.dot(diff)
    sigma1_trace = np.trace(sigma1)
    sigma2_trace = np.trace(sigma2)

    return cal_diff + sigma1_trace + np.trace(sigma2) - 2*tr_covmean
...
def _handle_path(path, sess, low_profile=False):
    if path.endswith('.npz'): # fid_stat = .npz
        f = np.load(path)
        m, s = f['mu'][:], f['sigma'][:]
        f.close()
    else: # fid_buffer_dir, img_list from get_is
        path = pathlib.Path(path)
        files = list(path.glob('*.jpg')) + list(path.glob('*.png'))
        if low_profile:
            m, s = calculate_activation_statistics_from_files(files, sess)
        else: # normally low_profile = false
...
def calculate_fid_given_paths(paths, inception_path, low_profile=False):
    ...
    with tf.Session(config=config) as sess:
        ...
        m1, s1 = _handle_path(paths[0], sess, low_profile=low_profile) # paths[0] = fid_buffer_dir
        m2, s2 = _handle_path(paths[1], sess, low_profile=low_profile) # paths[1] = fid_stat <== default value from .npz
...
def get_activations2(images, model, batch_size=50, verbose=False):
    d0 = images.shape[0] # 5000
...
def calculate_activation_statistics2(images, model, batch_size=50, verbose=False):
...
def _handle_path2(path, low_profile=False):
    if path.endswith('.npz'): # fid_stat = .npz
        f = np.load(path)
        m, s = f['mu'][:], f['sigma'][:]
        f.close()
...
def calculate_fid_given_paths2(paths, inception_path, low_profile=False):
    """ Calculates the FID of two paths. """
    ...
    m2, s2 = _handle_path2(paths[1], low_profile=low_profile) 
