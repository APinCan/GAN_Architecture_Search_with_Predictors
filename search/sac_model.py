...
class QNetwork(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_dim):
        ...
        self.linear1 = nn.Linear(num_inputs + num_actions, hidden_dim).to(device)
        self.linear2 = nn.Linear(hidden_dim, hidden_dim).to(device)
        self.linear3 = nn.Linear(hidden_dim, 1).to(device)

        # Q2 architecture
        self.linear4 = nn.Linear(num_inputs + num_actions, hidden_dim).to(device)
        self.linear5 = nn.Linear(hidden_dim, hidden_dim).to(device)
        self.linear6 = nn.Linear(hidden_dim, 1).to(device)
...
class GaussianPolicy(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_dim, action_space=None):
        ...
        self.linear1 = nn.Linear(num_inputs, hidden_dim).to(device)
        self.linear2 = nn.Linear(hidden_dim, hidden_dim).to(device)

        self.mean_linear_1 = nn.Linear(hidden_dim, len(CONV_TYPE)).to(device)
        self.mean_linear_2 = nn.Linear(hidden_dim, len(NORM_TYPE)).to(device)
        self.mean_linear_3 = nn.Linear(hidden_dim, len(UP_TYPE)).to(device)
        self.mean_linear_4 = nn.Linear(hidden_dim, len(SHORT_CUT_TYPE)).to(device)
        self.mean_linear_5 = nn.Linear(hidden_dim, len(SKIP_TYPE)).to(device)
        self.mean_linear_6 = nn.Linear(hidden_dim, len(SKIP_TYPE)).to(device)

        self.log_std_linear_1 = nn.Linear(hidden_dim, len(CONV_TYPE)).to(device)
        self.log_std_linear_2 = nn.Linear(hidden_dim, len(NORM_TYPE)).to(device)
        self.log_std_linear_3 = nn.Linear(hidden_dim, len(UP_TYPE)).to(device)
        self.log_std_linear_4 = nn.Linear(hidden_dim, len(SHORT_CUT_TYPE)).to(device)
        self.log_std_linear_5 = nn.Linear(hidden_dim, len(SKIP_TYPE)).to(device)
        self.log_std_linear_6 = nn.Linear(hidden_dim, len(SKIP_TYPE)).to(device)
    ...
    def sample(self, state, max_iter=1):
        ...
        if max_iter>1:
            for i in range(max_iter):
                return_eval.append([normal_1.rsample(), normal_2.rsample(), normal_3.rsample(), normal_4.rsample(), normal_5.rsample(), normal_6.rsample()])
        ...
        if max_iter>1:
            return a1,a2,a3,a4,a5,a6,log_prob_1,log_prob_2,log_prob_3,log_prob_4,log_prob_5,log_prob_6,mean_1,mean_2,mean_3,mean_4,mean_5,mean_6, return_eval
